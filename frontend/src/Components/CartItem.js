...
const CartItem = ({ item }) => {
  ...
  const handleQuantityChange = async (newQuantity) => {
    try {
      await axios.put(`http://localhost:8000/Patient/updateCartItem/${item._id}`, { quantity: newQuantity });
  ...
  const handleDeleteCartItem = async () => {
    try {
      await axios.delete(`http://localhost:8000/Patient/deleteCartItem/${item._id}`);
    } catch (error) {
